...
resource "aws_sqs_queue_policy" "deadletter_queue" {
  count     = var.enable_dlq ? 1 : 0
  queue_url = aws_sqs_queue.deadletter_queue[count.index].id
  policy    = data.aws_iam_policy_document.deadletter_queue[count.index].json
}
...
resource "aws_cloudwatch_metric_alarm" "deadletter_alarm" {
  count               = var.enable_dlq ? 1 : 0
  alarm_name          = "${aws_sqs_queue.deadletter_queue[count.index].name}-not-empty-alarm"
  alarm_description   = "Items are on the ${aws_sqs_queue.deadletter_queue[count.index].name} queue"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
  metric_name         = "ApproximateNumberOfMessagesVisible"
  namespace           = "AWS/SQS"
  ...
  tags                = tomap(local.tags)
  dimensions = {
    "QueueName" = aws_sqs_queue.deadletter_queue[count.index].name
  }
}
